...
- Use constructors to initialize structs if they are more convenient, i.e. less fields required, sets defaults, etc.
...
- Use meaningful names. This is not C. But also not Java. Getters should not be prefixed with `Get`, see [effective_go](https://go.dev/doc/effective_go).
